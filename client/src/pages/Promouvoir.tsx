...
export default function Promouvoir() {
  ...
  const { data: widget, isLoading } = useQuery<WidgetConfiguration | null>({
    ...
    queryFn: async () => {
      const res = await fetch("/api/professional/widget", {
        credentials: "include",
      });
      if (res.status === 404) {
        return null;
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description:
          "Votre bouton de réservation a été mis à jour avec succès.",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const checkSlugAvailability = async (slug: string) => {
    ...
    try {
      const response = await fetch(
        `/api/professional/widget/check-slug/${slug}`,
      );
  ...
  const bookingUrl = widget?.slug
    ? `${window.location.origin}/rdv/${widget.slug}`
    : "";
      ...
      <div className="space-y-4">
        ...
        <div className="text-center space-y-2 py-3">
          ...
          <p className="text-sm text-muted-foreground max-w-2xl mx-auto">
            ...
            et intégrez-le en quelques clics
          </p>
        </div>
                ...
                <Card>
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="text-xs">
                      Configurez l'apparence et le comportement de votre bouton
                      de réservation
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                {window.location.origin}/rdv/
                                {field.value || "votre-slug"}
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormControl>
                                <Input
                                  {...field}
                                  placeholder="Clinique de Santé Globale"
                                  data-testid="input-display-name"
                                />
                              </FormControl>
                              <FormDescription>
                                Le nom qui apparaîtra sur votre page de
                                réservation (laissez vide pour utiliser le nom
                                de votre clinique/profil)
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                          ...
                          render={({ field }) => (
                            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                              <div className="space-y-0.5">
                                ...
                                <FormDescription>
                                  S'il y'a plus de un professionnel, les clients
                                  pourront choisir le premier disponible
                                </FormDescription>
                              </div>
                              <FormControl>
                                <Switch
                          ...
                          render={({ field }) => (
                            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                              <div className="space-y-0.5">
                                ...
                                <FormDescription>
                                  Activez ou désactivez votre widget de
                                  réservation
                                </FormDescription>
                              </div>
                              <FormControl>
              ...
              <TabsContent value="integration" className="mt-4">
                <Card>
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="text-xs">
                      Choisissez la méthode d'intégration qui vous convient le
                      mieux
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <h3 className="text-sm font-semibold">
                        Bouton HTML (lien externe)
                      </h3>
                      <p className="text-sm text-muted-foreground">
                        Ajoutez un bouton cliquable qui ouvre la page de
                        réservation dans un nouvel onglet
                      </p>
                    ...
                    <div className="space-y-3">
                      <h3 className="text-sm font-semibold">
                        Intégration complète (iFrame)
                      </h3>
                      <p className="text-sm text-muted-foreground">
                        Intégrez la page de réservation directement dans votre
                        site
                      </p>
                      <div className="rounded-lg border p-4 bg-muted/50">
                        <code className="text-xs block mb-3 whitespace-pre-wrap break-all">
                    ...
                    <div className="rounded-lg border p-4 bg-blue-50 dark:bg-blue-950">
                      ...
                      <ol className="list-decimal list-inside space-y-1 text-sm text-muted-foreground">
                        <li>
                          Choisissez l'option qui convient (URL, bouton ou
                          iFrame)
                        </li>
                        <li>Copiez le code en cliquant sur "Copier le code"</li>
                        <li>Collez-le dans votre site web</li>
                        <li>
                          C'est prêt ! Vos clients peuvent maintenant prendre
                          rendez-vous
                        </li>
                      </ol>
                    </div>
